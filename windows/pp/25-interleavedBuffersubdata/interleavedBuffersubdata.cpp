...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInsatnce, LPSTR lpszCmdLine, int nCmdShow)
{
    ...
    MSG message;
    TCHAR szApplicationTitle[] = TEXT("CG - PP - Interleaved Cube using buffersubdata");
    TCHAR szApplicationClassName[] = TEXT("RTR_OPENGL_PP_INTERLEAVED_CUBE_BUFFERSUBDATA");
    bool done = false;
...
void initializeCubeBuffers()
{
    ...
    GLsizeiptr sizeofCubeVertices = sizeof(cubeVertices);
    GLsizeiptr sizeofCubeNormals = sizeof(cubeNormals);
    GLsizeiptr sizeofCubeColors = sizeof(cubeColors);
    GLsizeiptr sizeofCubeTextureCoordinates = sizeof(cubeTextureCoordinates);
    GLsizeiptr totalSize = sizeofCubeVertices + sizeofCubeNormals + sizeofCubeColors + sizeofCubeTextureCoordinates;
    GLuint offset = 0;
    ...
    glBufferSubData(GL_ARRAY_BUFFER, offset, sizeofCubeVertices, cubeVertices);
    offset += sizeofCubeVertices;

    glBufferSubData(GL_ARRAY_BUFFER, offset, sizeofCubeNormals, cubeNormals);
    offset += sizeofCubeNormals;

    glBufferSubData(GL_ARRAY_BUFFER, offset, sizeofCubeColors, cubeColors);
    offset += sizeofCubeColors;

    glBufferSubData(GL_ARRAY_BUFFER, offset, sizeofCubeTextureCoordinates, cubeTextureCoordinates);
    offset += sizeofCubeTextureCoordinates;

    glVertexAttribPointer(CG_ATTRIBUTE_VERTEX_POSITION, 3, GL_FLOAT, GL_FALSE, 0, (GLvoid *)0);
    glEnableVertexAttribArray(CG_ATTRIBUTE_VERTEX_POSITION);

    glVertexAttribPointer(CG_ATTRIBUTE_NORMAL, 3, GL_FLOAT, GL_FALSE, 0, (GLvoid *)sizeofCubeVertices);
    glEnableVertexAttribArray(CG_ATTRIBUTE_NORMAL);

    glVertexAttribPointer(CG_ATTRIBUTE_COLOR, 3, GL_FLOAT, GL_FALSE, 0, (GLvoid *)(sizeofCubeVertices + sizeofCubeNormals));
    glEnableVertexAttribArray(CG_ATTRIBUTE_COLOR);

    glVertexAttribPointer(CG_ATTRIBUTE_TEXTURE0, 2, GL_FLOAT, GL_FALSE, 0, (GLvoid *)(sizeofCubeVertices + sizeofCubeNormals + sizeofCubeColors));
