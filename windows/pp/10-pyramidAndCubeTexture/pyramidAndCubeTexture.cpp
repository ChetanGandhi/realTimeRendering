...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInsatnce, LPSTR lpszCmdLine, int nCmdShow)
{
    ...
    MSG message;
    TCHAR szApplicationTitle[] = TEXT("CG - PP - Pyramid and Cube Texture");
    TCHAR szApplicationClassName[] = TEXT("RTR_OPENGL_PP_PYRAMID_CUBE_TEXTURE");
    bool done = false;
...
void initialize(void)
{
    ...
    loadGLTextures(&textureKundali, MAKEINTRESOURCE(VIJAY_KUNDALI_BMP));
	loadGLTextures(&textureStone, MAKEINTRESOURCE(STONE_BMP));
