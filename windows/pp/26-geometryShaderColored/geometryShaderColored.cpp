...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInsatnce, LPSTR lpszCmdLine, int nCmdShow)
{
    ...
    MSG message;
    TCHAR szApplicationTitle[] = TEXT("CG - PP - Geometry Shader Colored");
    TCHAR szApplicationClassName[] = TEXT("RTR_OPENGL_PP_GEOMETRY_SHADER_COLORED");
    bool done = false;
...
void initializeGeometryShader()
{
    ...
    const GLchar *geometryShaderCode = "#version 430 core" \
        ...
        "       outVertexColor = vec4(0.0, 0.0, 0.0, 1.0);" \
        "       outVertexColor[colorIndex] = 1.0;" \
        "       EmitVertex();" \
