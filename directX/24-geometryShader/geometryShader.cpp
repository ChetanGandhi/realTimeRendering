...
ID3D11GeometryShader *geometryShaderObject = NULL;
...
HRESULT initializeGeometryShader(ID3DBlob **geometryShaderCode);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInsatnce, LPSTR lpszCmdLine, int nCmdShow)
{
    ...
    MSG message;
    TCHAR szApplicationTitle[] = TEXT("CG - Geometry Shader");
    TCHAR szApplicationClassName[] = TEXT("RTR_D3D_GEOMETRY_SHADER");
    bool done = false;
...
HRESULT initialize(void)
{
    ...
    ID3DBlob *geometryShaderCode = NULL;
    ...
    result = initializeGeometryShader(&geometryShaderCode);
...
HRESULT initializeGeometryShader(ID3DBlob **geometryShaderCode)
{
    ...
    HRESULT result = D3DCompileFromFile(L"./shaders/geometryShader.hlsl",
        NULL,
        D3D_COMPILE_STANDARD_FILE_INCLUDE,
        "main",
        "gs_5_0",
    ...
    if(FAILED(result))
    {
        if(error != NULL)
        {
            log("[Error] | Failed to compile geometry shader, error message: %s\n", (char *)error->GetBufferPointer());
            error->Release();
            error = NULL;
        }
        else
        {
            log("[Error] | Failed to compile geometry shader, error code: %#010x\n", result);
        }

        return result;
    }

    result = device->CreateGeometryShader((*geometryShaderCode)->GetBufferPointer(),
        (*geometryShaderCode)->GetBufferSize(),
        NULL,
    ...
    if(FAILED(result))
    {
        log("[Error] | Failed to create geometry shader, error code: %#010x\n", result);
        return result;
    }

    deviceContext->GSSetShader(geometryShaderObject, 0, 0);
