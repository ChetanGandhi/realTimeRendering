...
ID3D11Buffer *vertexBufferPyramidTexture = NULL;
ID3D11Buffer *vertexBufferCubePosition = NULL;
ID3D11Buffer *vertexBufferCubeTexture = NULL;
ID3D11Buffer *constantBuffer = NULL;
ID3D11ShaderResourceView *shaderResourceViewPyramidTexture = NULL;
ID3D11ShaderResourceView *shaderResourceViewCubeTexture = NULL;
ID3D11SamplerState *samplerStatePyramidTexture = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInsatnce, LPSTR lpszCmdLine, int nCmdShow)
{
    ...
    MSG message;
    TCHAR szApplicationTitle[] = TEXT("CG - Pyramid and Cube Texture");
    TCHAR szApplicationClassName[] = TEXT("RTR_D3D_PYRAMID_CUBE_TEXTURE");
    bool done = false;
...
HRESULT initialize(void)
{
   ...
   if(vertexShaderCode != NULL)
    ...
    result = loadTexture(L"resources/stone.bmp", &samplerStatePyramidTexture,  &shaderResourceViewPyramidTexture);
    ...
    result = loadTexture(L"./resources/vijay_kundali.bmp", &samplerStateCubeTexture, &shaderResourceViewCubeTexture);
...
HRESULT initializePyramidBuffers(void)
{
    ...
    const float pyramidTextureCoordinates[] = {
		0.5, 1.0,
		1.0, 0.0,
		0.0, 0.0,
    ...
    vertexBufferDescriptor.Usage = D3D11_USAGE_DYNAMIC;
    vertexBufferDescriptor.ByteWidth = sizeof(float) * ARRAYSIZE(pyramidTextureCoordinates);
    vertexBufferDescriptor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferDescriptor.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

    result = device->CreateBuffer(&vertexBufferDescriptor, nullptr, &vertexBufferPyramidTexture);
    ...
    deviceContext->Map(vertexBufferPyramidTexture, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
    memcpy(mappedSubresource.pData, pyramidTextureCoordinates, sizeof(pyramidTextureCoordinates));
    deviceContext->Unmap(vertexBufferPyramidTexture, NULL);
...
HRESULT initializeCubeBuffers(void)
{
    ...
    vertexBufferDescriptor.Usage = D3D11_USAGE_DYNAMIC;
    vertexBufferDescriptor.ByteWidth = sizeof(float) * ARRAYSIZE(cubeTextureCoordinates);
    vertexBufferDescriptor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferDescriptor.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

    result = device->CreateBuffer(&vertexBufferDescriptor, nullptr, &vertexBufferCubeTexture);
    ...
    deviceContext->Map(vertexBufferCubeTexture, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
    memcpy(mappedSubresource.pData, cubeTextureCoordinates, sizeof(cubeTextureCoordinates));
    deviceContext->Unmap(vertexBufferCubeTexture, NULL);
...
void drawPyramid(void)
{
    ...
    deviceContext->IASetVertexBuffers(CG_INPUT_SLOT_TEXTURE, 1, &vertexBufferPyramidTexture, &textureStride, &textureOffset);
    deviceContext->PSSetShaderResources(0, 1, &shaderResourceViewPyramidTexture);
    deviceContext->PSSetSamplers(0, 1, &samplerStatePyramidTexture);
...
void drawCube(void)
{
    ...
    deviceContext->IASetVertexBuffers(CG_INPUT_SLOT_TEXTURE, 1, &vertexBufferCubeTexture, &textureStride, &textureOffset);
...
void cleanUp(void)
{
    ...
    if(samplerStatePyramidTexture != NULL)
    {
        samplerStatePyramidTexture->Release();
        samplerStatePyramidTexture = NULL;
    }

    if(shaderResourceViewPyramidTexture != NULL)
    {
        shaderResourceViewPyramidTexture->Release();
        shaderResourceViewPyramidTexture = NULL;
    }
    ...
    if(vertexBufferPyramidTexture != NULL)
    {
        vertexBufferPyramidTexture->Release();
        vertexBufferPyramidTexture = NULL;
    }
    ...
    if(vertexBufferCubeTexture != NULL)
    {
        vertexBufferCubeTexture->Release();
        vertexBufferCubeTexture = NULL;
    }
