...
bool isPerPixelLightEnabled = false;
...
ID3D11VertexShader *vertexShaderObjectPerVertex = NULL;
ID3D11PixelShader *pixelShaderObjectPerVertex = NULL;
ID3D11VertexShader *vertexShaderObjectPerPixel = NULL;
ID3D11PixelShader *pixelShaderObjectPerPixel = NULL;
ID3D11InputLayout *inputLayoutPerVertex = NULL;
ID3D11InputLayout *inputLayoutPerPixel = NULL;
...
HRESULT initializeVertexShader(LPCWSTR fileName, ID3DBlob **vertexShaderCode, ID3D11VertexShader **vertexShaderObject);
HRESULT initializePixelShader(LPCWSTR fileName, ID3DBlob **pixelShaderCode, ID3D11PixelShader **pixelShaderObject);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInsatnce, LPSTR lpszCmdLine, int nCmdShow)
{
    ...
    MSG message;
    TCHAR szApplicationTitle[] = TEXT("CG - Per Vertex and Pixel Light");
    TCHAR szApplicationClassName[] = TEXT("RTR_PER_VERTEX_PIXEL_LIGHT");
    bool done = false;
    ...
    switch(iMessage)
    {
        ...
        case WM_CHAR:
            switch(wParam)
            {
                case 'P':
                case 'p':
                    isPerPixelLightEnabled = true;
                    deviceContext->VSSetShader(vertexShaderObjectPerPixel, 0, 0);
                    deviceContext->PSSetShader(pixelShaderObjectPerPixel, 0, 0);
                    deviceContext->IASetInputLayout(inputLayoutPerPixel);
                break;
                ...
                case 'V':
                case 'v':
                    isPerPixelLightEnabled = false;
                    deviceContext->VSSetShader(vertexShaderObjectPerVertex, 0, 0);
                    deviceContext->PSSetShader(pixelShaderObjectPerVertex, 0, 0);
                    deviceContext->IASetInputLayout(inputLayoutPerVertex);
                break;
...
HRESULT initialize(void)
{
    HRESULT result = initializeSwapChain();
...
{
    HRESULT result = S_OK;
    ID3DBlob *vertexShaderCodePerVertex = NULL;
    ID3DBlob *pixelShaderCodePerVertex = NULL;

    result = initializeVertexShader(L"./shaders/vertexShaderPerVertex.hlsl",  &vertexShaderCodePerVertex, &vertexShaderObjectPerVertex);

    if(FAILED(result))
    {
        if(vertexShaderCodePerVertex != NULL)
        {
            vertexShaderCodePerVertex->Release();
            vertexShaderCodePerVertex = NULL;
        }

        return result;
    }

    result = initializePixelShader(L"./shaders/pixelShaderPerVertex.hlsl", &pixelShaderCodePerVertex, &pixelShaderObjectPerVertex);
    ...
    if(FAILED(result))
    {
        ...
        if(pixelShaderCodePerVertex != NULL)
        {
            pixelShaderCodePerVertex->Release();
            pixelShaderCodePerVertex = NULL;
        }

        return result;
    }

    result = initializeInputLayout(vertexShaderCodePerVertex, &inputLayoutPerVertex);
...
{
    HRESULT result = S_OK;
    ID3DBlob *vertexShaderCodePerPixel = NULL;
    ID3DBlob *pixelShaderCodePerPixel = NULL;

    result = initializeVertexShader(L"./shaders/vertexShaderPerPixel.hlsl",  &vertexShaderCodePerPixel, &vertexShaderObjectPerPixel);

    if(FAILED(result))
    {
        if(vertexShaderCodePerPixel != NULL)
        {
            vertexShaderCodePerPixel->Release();
            vertexShaderCodePerPixel = NULL;
        }

        return result;
    }

    result = initializePixelShader(L"./shaders/pixelShaderPerPixel.hlsl", &pixelShaderCodePerPixel, &pixelShaderObjectPerPixel);
    ...
    if(FAILED(result))
    {
        ...
        if(pixelShaderCodePerPixel != NULL)
        {
            pixelShaderCodePerPixel->Release();
            pixelShaderCodePerPixel = NULL;
        }

        return result;
    }

    result = initializeInputLayout(vertexShaderCodePerPixel, &inputLayoutPerPixel);
...
HRESULT initializeVertexShader(LPCWSTR fileName, ID3DBlob **vertexShaderCode, ID3D11VertexShader **vertexShaderObject)
{
    ...
    if(FAILED(result))
    {
        if(error != NULL)
        {
            log("[Error] | Failed to compile vertex shader '%s', error message: %s\n", fileName, (char *)error->GetBufferPointer());
            error->Release();
            error = NULL;
        }
        else
        {
            log("[Error] | Failed to compile vertex shader '%s', error code: %#010x\n", fileName, result);
        }
    ...
    if(FAILED(result))
    {
        log("[Error] | Failed to create vertex shader '%s', error code: %#010x\n", fileName, result);
        return result;
    }
...
HRESULT initializePixelShader(LPCWSTR fileName, ID3DBlob **pixelShaderCode, ID3D11PixelShader **pixelShaderObject)
{
    ...
    if(FAILED(result))
    {
        if(error != NULL)
        {
            log("[Error] | Failed to compile pixel shader '%s', error message: %s\n", fileName, (char *)error->GetBufferPointer());
            error->Release();
            error = NULL;
        }
        else
        {
            log("[Error] | Failed to compile pixel shader '%s', error code: %#010x\n", fileName, result);
        }
    ...
    if(FAILED(result))
    {
        log("[Error] | Failed to create pixel shader '%s', error code: %#010x\n", fileName, result);
        return result;
    }
...
void cleanUp(void)
{
    ...
    if(inputLayoutPerVertex != NULL)
    {
        inputLayoutPerVertex->Release();
        inputLayoutPerVertex = NULL;
    }

    if(inputLayoutPerPixel != NULL)
    {
        inputLayoutPerPixel->Release();
        inputLayoutPerPixel = NULL;
    }

    if(vertexShaderObjectPerVertex != NULL)
    {
        vertexShaderObjectPerVertex->Release();
        vertexShaderObjectPerVertex = NULL;
    }

    if(vertexShaderObjectPerPixel != NULL)
    {
        vertexShaderObjectPerPixel->Release();
        vertexShaderObjectPerPixel = NULL;
    }

    if(pixelShaderObjectPerVertex != NULL)
    {
        pixelShaderObjectPerVertex->Release();
        pixelShaderObjectPerVertex = NULL;
    }

    if(pixelShaderObjectPerPixel != NULL)
    {
        pixelShaderObjectPerPixel->Release();
        pixelShaderObjectPerPixel = NULL;
    }
