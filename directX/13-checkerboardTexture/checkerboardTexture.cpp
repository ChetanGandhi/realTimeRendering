...
ID3D11Buffer *vertexBufferSquareTexture = NULL;
ID3D11Buffer *constantBuffer = NULL;
ID3D11ShaderResourceView *shaderResourceViewCheckerboardTexture = NULL;
ID3D11SamplerState *samplerStateCheckerboardTexture = NULL;
...
HRESULT initializeCheckerboardBuffers(void);
HRESULT initializeConstantBuffers(void);
HRESULT disableBackFaceCulling(void);
HRESULT makeCheckerboardTexture(ID3D11SamplerState **samplerState, ID3D11ShaderResourceView **shaderResourceView);
UINT createBitmapImage(long imageWidth, long imageHeight, byte *imageColorData, UINT imageColorDataSize, byte **bitmap);
void update(void);
void display(void);
void drawSquare(float *squareVertices, size_t verticesSize);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInsatnce, LPSTR lpszCmdLine, int nCmdShow)
{
    ...
    MSG message;
    TCHAR szApplicationTitle[] = TEXT("CG - Checkerboard Texture");
    TCHAR szApplicationClassName[] = TEXT("RTR_D3D_CHECKERBOARD_TEXTURE");
    bool done = false;
...
HRESULT initialize(void)
{
   ...
   if(vertexShaderCode != NULL)
    ...
    result = initializeCheckerboardBuffers();
    ...
    result = makeCheckerboardTexture(&samplerStateCheckerboardTexture,  &shaderResourceViewCheckerboardTexture);
...
HRESULT initializeCheckerboardBuffers(void)
{
    ...
    vertexBufferDescriptor.ByteWidth = sizeof(float) * 18;
    ...
    vertexBufferDescriptor.Usage = D3D11_USAGE_DYNAMIC;
    vertexBufferDescriptor.ByteWidth = sizeof(float) * ARRAYSIZE(squareTextureCoordinates);
    vertexBufferDescriptor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferDescriptor.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

    result = device->CreateBuffer(&vertexBufferDescriptor, nullptr, &vertexBufferSquareTexture);
    ...
    deviceContext->Map(vertexBufferSquareTexture, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
    memcpy(mappedSubresource.pData, squareTextureCoordinates, sizeof(squareTextureCoordinates));
    deviceContext->Unmap(vertexBufferSquareTexture, NULL);
...
void display(void)
{
    ...
    drawSquare(straightSquareVertices, sizeof(straightSquareVertices));
    drawSquare(tiltedSquareVertices, sizeof(tiltedSquareVertices));
...
void drawSquare(float *squareVertices, size_t verticesSize)
{
    ...
    memcpy(mappedSubresource.pData, squareVertices, verticesSize);
    ...
    deviceContext->IASetVertexBuffers(CG_INPUT_SLOT_TEXTURE, 1, &vertexBufferSquareTexture, &textureStride, &textureOffset);
    deviceContext->PSSetShaderResources(0, 1, &shaderResourceViewCheckerboardTexture);
    deviceContext->PSSetSamplers(0, 1, &samplerStateCheckerboardTexture);
...
HRESULT makeCheckerboardTexture(ID3D11SamplerState **samplerState, ID3D11ShaderResourceView **shaderResourceView)
{
	...
	for (heightCounter = 0; heightCounter < checkImageHeight; ++heightCounter)
	{
		for (widthCounter = 0; widthCounter < checkImageWidth; ++widthCounter)
		{
			...
			checkImage[heightCounter][widthCounter][0] = (byte)colorWhite;
			checkImage[heightCounter][widthCounter][1] = (byte)colorWhite;
			checkImage[heightCounter][widthCounter][2] = (byte)colorWhite;
			checkImage[heightCounter][widthCounter][3] = (byte)255;
		}
	}

    byte *bitmapImage = NULL;
    UINT totalBitmapImageSize = createBitmapImage(checkImageWidth, checkImageHeight, (byte *)checkImage, sizeof(checkImage), &bitmapImage);
...
HRESULT resize(int width, int height)
{
    ...
    perspectiveProjectionMatrix= XMMatrixPerspectiveFovLH(XMConvertToRadians(60.0f), (float)width/(float)height, 0.1f, 30.0f);
...
void cleanUp(void)
{
    ...
    if(samplerStateCheckerboardTexture != NULL)
    {
        samplerStateCheckerboardTexture->Release();
        samplerStateCheckerboardTexture = NULL;
    }

    if(shaderResourceViewCheckerboardTexture != NULL)
    {
        shaderResourceViewCheckerboardTexture->Release();
        shaderResourceViewCheckerboardTexture = NULL;
    }
    ...
    if(vertexBufferSquareTexture != NULL)
    {
        vertexBufferSquareTexture->Release();
        vertexBufferSquareTexture = NULL;
    }
