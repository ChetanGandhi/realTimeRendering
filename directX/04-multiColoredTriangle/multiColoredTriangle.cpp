...
ID3D11Buffer *vertexBufferPosition = NULL;
ID3D11Buffer *vertexBufferColor = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInsatnce, LPSTR lpszCmdLine, int nCmdShow)
{
    ...
    MSG message;
    TCHAR szApplicationTitle[] = TEXT("CG - Multi Colored Triangle");
    TCHAR szApplicationClassName[] = TEXT("RTR_D3D_MULTI_COLORED_TRIANGLE");
    bool done = false;
...
HRESULT initializeVertexBuffer(void)
{
    ...
    HRESULT result = device->CreateBuffer(&vertexBufferDescriptor, nullptr, &vertexBufferPosition);
    ...
    deviceContext->Map(vertexBufferPosition, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
    memcpy(mappedSubresource.pData, triangleVertices, sizeof(triangleVertices));
    deviceContext->Unmap(vertexBufferPosition, NULL);
    ...
    result = device->CreateBuffer(&vertexBufferDescriptor, nullptr, &vertexBufferColor);
    ...
    deviceContext->Map(vertexBufferColor, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
    memcpy(mappedSubresource.pData, triangleColors, sizeof(triangleColors));
    deviceContext->Unmap(vertexBufferColor, NULL);
...
void display(void)
{
    ...
    deviceContext->IASetVertexBuffers(CG_INPUT_SLOT_VERTEX_POSITION, 1, &vertexBufferPosition, &stride, &offset);
    deviceContext->IASetVertexBuffers(CG_INPUT_SLOT_COLOR, 1, &vertexBufferColor, &stride, &offset);
...
void cleanUp(void)
{
    ...
    if(vertexBufferPosition != NULL)
    {
        ...
        vertexBufferPosition = NULL;
    }

    if(vertexBufferColor != NULL)
    {
        vertexBufferColor->Release();
        vertexBufferColor = NULL;
    }
