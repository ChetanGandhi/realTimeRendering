...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInsatnce, LPSTR lpszCmdLine, int nCmdShow)
{
    ...
    MSG message;
    TCHAR szApplicationTitle[] = TEXT("CG - Triangle Ortho");
    TCHAR szApplicationClassName[] = TEXT("RTR_D3D_TRIANGLE_ORTHO");
    bool done = false;
...
void display(void)
{
    ...
    XMMATRIX worldViewProjectionMatrix = worldMatrix * viewMatrix * orthographicProjectionMatrix;
...
HRESULT resize(int width, int height)
{
    ...
    if(width <= height)
    {
        ...
        orthographicProjectionMatrix= XMMatrixOrthographicOffCenterLH(-100.0f, 100.0f, -100.0f * ratio, 100.0f * ratio, -100.0f, 100.0f);
    }
    else
    {
        ratio = (float)width / (float)height;
        orthographicProjectionMatrix= XMMatrixOrthographicOffCenterLH(-100.0f * ratio, 100.0f * ratio, -100.0f, 100.0f, -100.0f, 100.0f);
    }
