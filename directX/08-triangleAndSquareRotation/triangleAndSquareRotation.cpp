...
ID3D11Buffer *vertexBufferTriangleColor = NULL;
ID3D11Buffer *vertexBufferSquarePosition = NULL;
ID3D11Buffer *vertexBufferSquareColor = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInsatnce, LPSTR lpszCmdLine, int nCmdShow)
{
    ...
    MSG message;
    TCHAR szApplicationTitle[] = TEXT("CG - Triangle and Square Rotation");
    TCHAR szApplicationClassName[] = TEXT("RTR_D3D_TRIANGLE_SQUARE_ROTATION");
    bool done = false;
...
HRESULT initializeTriangleBuffers(void)
{
    ...
    vertexBufferDescriptor.Usage = D3D11_USAGE_DYNAMIC;
    vertexBufferDescriptor.ByteWidth = sizeof(float) * ARRAYSIZE(triangleColors);
    vertexBufferDescriptor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferDescriptor.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

    result = device->CreateBuffer(&vertexBufferDescriptor, nullptr, &vertexBufferTriangleColor);
    ...
    deviceContext->Map(vertexBufferTriangleColor, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
    memcpy(mappedSubresource.pData, triangleColors, sizeof(triangleColors));
    deviceContext->Unmap(vertexBufferTriangleColor, NULL);
...
HRESULT initializeSquareBuffers(void)
{
    ...
    vertexBufferDescriptor.Usage = D3D11_USAGE_DYNAMIC;
    vertexBufferDescriptor.ByteWidth = sizeof(float) * ARRAYSIZE(squareColors);
    vertexBufferDescriptor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferDescriptor.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

    result = device->CreateBuffer(&vertexBufferDescriptor, nullptr, &vertexBufferSquareColor);
    ...
    deviceContext->Map(vertexBufferSquareColor, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
    memcpy(mappedSubresource.pData, squareColors, sizeof(squareColors));
    deviceContext->Unmap(vertexBufferSquareColor, NULL);
...
void drawTriangle(void)
{
    ...
    deviceContext->IASetVertexBuffers(CG_INPUT_SLOT_COLOR, 1, &vertexBufferTriangleColor, &stride, &offset);
    ...
    float angleInRadians = XMConvertToRadians(angleTriangle);
...
void drawSquare(void)
{
    ...
    deviceContext->IASetVertexBuffers(CG_INPUT_SLOT_COLOR, 1, &vertexBufferSquareColor, &stride, &offset);
    deviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    ...
    float angleInRadians = XMConvertToRadians(angleSquare);

    rotationMatrix = XMMatrixRotationX(angleInRadians);
    worldMatrix = rotationMatrix * translationMatrix;
    XMMATRIX worldViewProjectionMatrix = worldMatrix * viewMatrix * perspectiveProjectionMatrix;
...
void cleanUp(void)
{
    ...
    if(vertexBufferTriangleColor != NULL)
    {
        vertexBufferTriangleColor->Release();
        vertexBufferTriangleColor = NULL;
    }
    ...
    if(vertexBufferSquareColor != NULL)
    {
        vertexBufferSquareColor->Release();
        vertexBufferSquareColor = NULL;
    }
