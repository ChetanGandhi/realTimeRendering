...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInsatnce, LPSTR lpszCmdLine, int nCmdShow)
{
    ...
    MSG message;
    TCHAR szApplicationTitle[] = TEXT("CG - Deathly Hallows");
    TCHAR szApplicationClassName[] = TEXT("RTR_OPENGL_DEATHLY_HALLOWS");
    bool done = false;
...
void drawResurrectionStone(void)
{
    ...
    for (int counter = 0; counter < circlePoints; ++counter)
    {
        ...
        glVertex3f(cos(angle) * 0.31f, (sin(angle) * 0.31f) - 0.19f, 0.0f);
    }
