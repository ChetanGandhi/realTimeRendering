package com.github.chetangandhi.smileyTweaked;
...
public class SmileyTweaked extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener {
    ...
    private int[] textureWhiteColor = new int[1];
    private int[] textureSmiley = new int[1];

    private float[] perspectiveProjectionMatrix = new float[16];

    SmileyTweaked(Context context) {
        super(context);
    ...
    public boolean onSingleTapConfirmed(MotionEvent event) {
        ...
        if(this.textureMode < 0 || this.textureMode > 4) {
            this.textureMode = 0;
        }
    ...
    private void initializeSmileyBuffers() {
        ...
        // i.e. 8 * 4, 8 is number of coordinates in array.
    ...
    private void drawSmiley() {
        ...
        float[] smileyTextureCoordinates = new float[8];
        int[] currentTexture = textureSmiley;
        ...
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, 8 * 4, textureCoordinatesBuffer, GLES32.GL_DYNAMIC_DRAW);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);
    ...
    void makeWhiteColorTexture() {
        int []colors = {android.graphics.Color.WHITE};
        Bitmap bitmap = Bitmap.createBitmap(colors, 1, 1, Bitmap.Config.ARGB_8888);

        GLES32.glGenTextures(1, this.textureWhiteColor, 0);
        GLES32.glPixelStorei(GLES32.GL_UNPACK_ALIGNMENT, 1);
        GLES32.glBindTexture(GLES32.GL_TEXTURE_2D, this.textureWhiteColor[0]);
    ...
    private void cleanUp() {
        ...
        if (this.textureWhiteColor[0] != 0) {
            GLES32.glDeleteTextures(1, this.textureWhiteColor, 0);
            this.textureWhiteColor[0] = 0;
        }
    }
}
